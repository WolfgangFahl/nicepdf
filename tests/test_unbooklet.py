...
from nicepdf.pdftool import PdfFile, PDFTool
...
class TestPDFTool(Basetest):
    ...
    def create_booklet(
        self, double_pages: int = 2, postfix="", do_rotate: bool = False
    ):
        """
        create a booklet
        """
    ...
    def check_split(self, pdf_file, expected_pages: int):
        ...
        with open(output_path, "rb") as output_file:
            ...
            self.assertEqual(
                len(pdf.pages), expected_pages
            )  # 2 double pages yield 4 single pages

    def test_double_pages(self):
        """
        test the double page /half_page creation code
        """
        test_cases = [(4, [4, 1, 2, 3]), (100, [100, 1, 2, 99, 98, 3, 4, 97])]
    ...
    def test_split_booklet_style(self):
        ...
        for do_rotate in [True, False]:
            ...
            for double_pages in range(2, 8, 2):
                booklet = self.create_booklet(
                    double_pages, postfix=postfix, do_rotate=do_rotate
                )
